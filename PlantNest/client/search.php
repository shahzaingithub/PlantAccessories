...
$cat = $_GET['cat'];
if($cat ==  '0'){
    $query  = "SELECT * FROM products WHERE Name  LIKE '%$game%'";
}else{
    $query  = "SELECT * FROM products WHERE Category = '$cat' AND Name LIKE '%$game%'";
...
<input type="hidden" name="product_image" value="<?=$fetch['Image_Url']?>">
    ...
    ?>
    <h2 class="text-center">No Resullt Found</h2>
    <?php
}
?>
